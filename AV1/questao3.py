...
def generate_numeric_attributes(df, text_column):
    ...
    # Calcular a média dos valores de Count para cada termo
    count_means = count_matrix.mean(axis=0).A1

    # Selecionar os top 10 termos com maior Count
    top_10_count_indices = count_means.argsort()[-10:][::-1]
    top_10_count_terms = [count_features[i] for i in top_10_count_indices]

    # Calcular a média dos valores de TF-IDF para cada termo
    tfidf_means = tfidf_matrix.mean(axis=0).A1

    # Selecionar os top 10 termos com maior TF-IDF
    ...
    # Plotar gráfico dos 10 termos com maior Count
    ...
    # Plotar gráfico dos 10 termos com maior TF-IDF
    plt.subplot(1, 2, 2)
