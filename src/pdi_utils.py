...
def load_shoes1():
    return plt.imread("imgs//E10b.bmp")

def load_shoes2():
    return plt.imread("imgs//E10a.bmp")
...
def load_light_dataset(dataset = "D:\\mega\\Drive\\Mobit\\trafic_light_testes\\dataset_mini_teste"):
    data = []
    label_data = []
    ...
    for classe in range(len(classes)):
        ...
        path = os.path.join(dataset, classes[classe])
        dir_list = list(os.listdir(path))

        for id, dir in enumerate(dir_list):
            img_path = os.path.join(path, dir)
            img_array = cv2.imread(os.path.join(img_path), cv2.IMREAD_ANYCOLOR)
